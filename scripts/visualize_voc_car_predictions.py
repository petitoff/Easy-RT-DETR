...
def visualize(args: argparse.Namespace) -> None:
    ...
    with torch.no_grad():
        for index in range(min(args.num_images, len(eval_dataset))):
            ...
            output_path = output_dir / f"voc_car_pred_{index:02d}.png"
            render_image(
                image_tensor=image,
                target=target,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render GT and predicted boxes for Pascal VOC car-only checkpoints.")
    parser.add_argument("--checkpoint", type=str, default="artifacts/voc_car_checkpoint.pt")
    parser.add_argument("--data-root", type=str, default="data/VOCdevkit/VOC2007")
    parser.add_argument("--split", type=str, default="trainval")
    parser.add_argument("--output-dir", type=str, default="artifacts/voc_car_vis")
